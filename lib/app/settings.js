...
exports.refresh = function (settings, info) {
	for (var table in info) {
		...
		for (var name in columns) {
			if (exists(settings[table].columns, name) || name === pk) continue;
			settings[table].columns.push(createColumn(name, columns[name]));
		}

		var columns = _.keyBy(settings[table].columns, 'name');
		columns.createdAt.editview.show = false;
		columns.createdAt.allowNull = true;
		columns.updatedAt.editview.show = false;
		columns.updatedAt.allowNull = true;
		columns[pk].editview.show = false;
		columns[pk].allowNull = true;

		settings[table].columns = _.map(columns, function(value) {
				return value;
		});
	}
